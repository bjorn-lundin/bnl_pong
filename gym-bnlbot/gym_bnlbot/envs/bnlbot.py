...
class Bnlbot(gym.Env):
  metadata = {"render_modes": ["human", "ansi", "ram", "rgb"], "render_fps": 4}
  DO_NOT_PLACE_BET=0
  DO_PLACE_BET=1
...
#        if mydir.is_dir() and dirname == '2m4f_Hcap':
    ...
    for i in self.racefile_list:
        print('filename',i)
    print('num files', len(self.racefile_list))
#    a=1/0
    print('__init__ stop')

  ##########################################

  def draw_odds(self, img,n,tic):
    tmp_img = img
    offset = (n * 5) + 1
    y = 0
    x = offset
  ...
  def get_observation_as_tics(self):
    ...
    ll2=[]
    for d in ll :
       tic= np.uint8(Bnlbot.ODDS_TO_TIC_DICT[str(d)])
       ll2.append(tic)
  ...
  def step(self, action):
    ...
    if not truncated:
      ...
      if action == Bnlbot.DO_PLACE_BET :
        #Strategy to only bet once per race
        terminated = True
        ...
        ticlist = self.get_observation_as_tics()
        for odds in ticlist:
            #print('step.odds', type(odds), odds )
